...
export function OnboardingFarmDetails() {
  ...
  const getValidationErrors = () => {
    ...
    if (!formData.state) {
      errors.push(localize('Select state', 'राज्य चुनें', 'राज्य निवडा'));
    }
    return errors;
  };
  ...
  const getCompletionPercent = () => {
    let filled = 0;
    if (formData.state) filled += 16;
    if (formData.landOwnership) filled += 16;
    if (formData.landSize > 0) filled += 16;
    if (formData.selectedCrops.length > 0) filled += 16;
    if (formData.selectedSeasons.length > 0) filled += 18;
    if (formData.irrigation.length > 0) filled += 18;
    return filled;
  };
      ...
      <div className="bg-white px-4 pt-10 pb-3 flex items-center gap-3 border-b border-gray-100 shadow-sm">
        ...
        <div className="flex-1 text-center">
          <p className="text-[13px] font-medium text-[#6B7280]">
            {localize('Step 2 of 3 — Farm Details', 'चरण 2 / 3 — खेती की जानकारी', 'पायरी 2 / 3 — शेती माहिती')}
          </p>
        </div>
        <div className="w-9" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-28">
        ...
        <div className="px-6 py-6">
          ...
          >
            ...
            <div className="flex justify-between mt-2">
              {['state', 'ownership', 'landSize', 'crops', 'seasons', 'irrigation'].map((field) => (
                <div
                  key={field}
        ...
        <div className="px-6 space-y-4">
          ...
          <motion.div
            ...
            className={`bg-white rounded-3xl p-5 shadow-sm border-2 transition-all relative ${isFieldValid('state') ? 'border-green-200' : 'border-gray-100'
              }`}
          >
            <div className={sectionTitleClass}>
              <div className="w-5 h-5 flex items-center justify-center text-[#F5A623] text-lg">📍</div>
              <h3 className="font-semibold text-[16px] text-[#1C1C1E]">
                {localize('Select State', 'राज्य चुनें', 'राज्य निवडा')}
            ...
            <div className="relative">
              <div
                ...
                className="w-full flex items-center justify-between px-4 py-4 rounded-2xl bg-[#F7F3EE] transition-all cursor-pointer border-2"
                style={{
                  borderColor: showStateDropdown ? '#F5A623' : 'transparent',
                }}
              >
                <span
                  className="text-[15px]"
                  style={{
                    color: formData.state ? '#1C1C1E' : '#6B7280',
                  }}
                >
                  {formData.state || localize('Select your state', 'अपना राज्य चुनें', 'तुमचे राज्य निवडा')}
                </span>
                <svg
                  width="20"
                  height="20"
              ...
              {showStateDropdown && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="absolute left-0 right-0 top-[66px] z-50 bg-white rounded-2xl border border-gray-100 shadow-xl overflow-hidden"
                  style={{ maxHeight: 260 }}
                >
                  <div className="sticky top-0 bg-white px-3 pt-3 pb-2 border-b border-gray-50/50">
                    <input
                      type="text"
                      placeholder={localize('Search...', 'खोजें...', 'शोधा...')}
                      value={stateSearch}
                      onChange={(e) => setStateSearch(e.target.value)}
                      autoFocus
                      className="w-full px-4 py-2.5 rounded-xl bg-[#F7F3EE] text-[14px] outline-none border-2 border-transparent focus:border-[#F5A623]/30 placeholder:text-[#6B7280]"
                    ...
                    {filteredStates.length > 0 ? (
                      filteredStates.map((state) => (
                        <button
                          ...
                          className="w-full text-left px-4 py-3 text-[14px] hover:bg-[#F5A623]/5 transition-colors flex items-center justify-between border-b border-gray-50 last:border-0"
                          style={{
                            color: formData.state === state ? '#F5A623' : '#1C1C1E',
                            fontWeight: formData.state === state ? 600 : 400,
                          }}
                        >
                          <span>{state}</span>
                      ...
                      <div className="px-4 py-6 text-center text-[13px] text-[#6B7280]">
                        {localize('No results', 'कोई परिणाम नहीं', 'निकाल नाही')}
                      </div>
                    )}
                  </div>
                </motion.div>
